...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const isSidebarHiddenPage = ['/login', '/setup', '/admin'].includes(location.pathname);
  ...
  return (
    <div className="flex min-h-screen bg-zinc-950 text-zinc-100 font-sans selection:bg-maroon selection:text-white">
      ...
      <main className="flex-1 h-screen overflow-hidden w-full relative">
        <div className={`h-full overflow-y-auto custom-scrollbar ${isSidebarHiddenPage ? '' : isVault ? 'md:pl-64' : 'pt-16 pb-32 md:pt-0 md:pb-0 md:pl-64'}`}>
          {/* Uniform Margin Container */}
          <div className={`${isSidebarHiddenPage ? 'w-full h-full flex items-center justify-center p-4' : isVault ? 'w-full h-full' : 'max-w-7xl mx-auto w-full px-4 py-6 md:px-12 md:py-12'}`}>
            {children}
          </div>
        </div>
      </main>
