      ...
      <main className="flex-1 h-screen overflow-hidden w-full relative">
        ...
        {isAdminPage && (
          <div className="fixed top-6 left-6 z-[110]">
            <Link
              to="/"
              className="flex items-center gap-2 px-4 py-2 bg-zinc-900/50 backdrop-blur-xl border border-zinc-800 rounded-xl text-zinc-400 hover:text-white hover:border-maroon/50 hover:bg-maroon/5 transition-all group shadow-2xl"
            >
        ...
        <div className={`h-full overflow-y-auto custom-scrollbar ${isAdminPage ? 'pt-24 pb-12' : 'pt-16 pb-32 md:pt-0 md:pb-0 md:pl-64'}`}>
          {/* Uniform Margin Container */}
          <div className={`max-w-7xl mx-auto w-full px-4 py-6 md:px-12 md:py-12 ${isAdminPage ? 'md:pt-6' : ''}`}>
            {children}
          </div>
        </div>
      </main>
